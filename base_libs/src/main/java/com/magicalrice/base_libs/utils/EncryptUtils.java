package com.magicalrice.base_libs.utils;
...
public final class EncryptUtils {
     ...
     * @return 16进制加盐密文
     */
    public static String encryptMD5ToString(final byte[] data, byte[] salt) {
        if (data == null || salt == null) return null;
        byte[] dataSalt = new byte[data.length + salt.length];
        System.arraycopy(data, 0, dataSalt, 0, data.length);
        System.arraycopy(salt, 0, dataSalt, 0, salt.length);
     ...
     * @return Base64密文
     */
    public static byte[] encryptDESToBase64(final byte[] data, final byte[] key) {
     ...
     * 3DES转变
     * 算法名称/加密模式/填充方式
     * 加密模式:电子密码本模式ECB、加密块链模式CBC、加密反馈模式CFB、输出反馈模式OFB
     * 填充方式：NoPadding、ZerosPadding、PKCS5Padding
     */
     ...
     * @return Base64密文
     */
    public static byte[] encrypt3DESToBase64(final byte[] data, final byte[] key) {
     ...
     * @return 16进制密文
     */
    public static String encrypt3DESToHexString(final byte[] data, final byte[] key) {
    ...
    public static byte[] encryptAESToBase64(final byte[] data, final byte[] key) {
     ...
     *
     * @param data 明文
     * @param key  16，24，32位字节密钥
     * @return 16进制密文
     */
    public static String encryptAESToHexString(final byte[] data, final byte[] key) {
    ...
    private static byte[] hexString2Bytes(String hexString) {
        ...
        for (int i = 0; i < len; i += 2) {
            ret[i >> 1] = (byte) (hex2Des(hexBytes[i]) << 4 | hex2Des(hexBytes[i + 1]));
        }
        return ret;
    }

    private static int hex2Des(final char hexChar) {
