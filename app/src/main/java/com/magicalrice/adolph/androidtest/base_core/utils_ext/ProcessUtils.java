...
public class ProcessUtils {
    ...
    public static String getForegroundProcessName() {
        ...
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (list.size() > 0) {
                try {
                    ApplicationInfo info = pm.getApplicationInfo(ApplicationUtils.getApp().getPackageName(), 0);
                    AppOpsManager aom = (AppOpsManager) ApplicationUtils.getApp().getSystemService(Context.APP_OPS_SERVICE);
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, info.uid, info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        ApplicationUtils.getApp().startActivity(intent);
                    }
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, info.uid, info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        Logger.e("getForegroundApp", "没有打开\"有权查看使用权限的应用\"选项");
                        return null;
                    }
                    UsageStatsManager usageStatsManager = (UsageStatsManager) ApplicationUtils.getApp().getSystemService(Context.USAGE_STATS_SERVICE);
                ...
                Logger.d("ProcessUtils", "getForegroundProcessName() called" + ": 无\"有权查看使用权限的应用选项\"");
            }
        }
        return null;
    }
