...
public class FailSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public FailSurfaceView(ScyScraperActivity context) {
        super(context);
    ...
    private void initBitmap() {
        bgAbout = BitmapFactory.decodeResource(getResources(), R.drawable.fail);
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (x < (screenWidth - (1 * picWidth)) + picWidth && x > (screenWidth - (1 * picWidth)) && y < screenHeight - 2 * picHeight + picHeight && y > screenHeight - 2 * picHeight) {
                    mActivity.mySv = new MySurfaceView(mActivity);
                    mActivity.setMenuView();
                    if (mActivity.isSound) {
                        mActivity.mpBack.start();
                        mActivity.mpWin.pause();
                    }
                }
                break;
        }
